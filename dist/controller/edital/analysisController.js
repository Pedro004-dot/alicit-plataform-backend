...
class EditalAnalysisController {
     ...
     */
    async buscarAnaliseDetalhada(req, res) {
        try {
            ...
            const { data: relatorio, error } = await supabase_1.supabase
     ...
     */
    async verificarRelatorioExistente(numeroControlePNCP, empresaCNPJ) {
        try {
     ...
     */
    async verificarAnaliseEmAndamento(empresaCNPJ) {
        try {
            const { data, error } = await supabase_1.supabase
     ...
     */
    async atualizarStatusLicitacao(numeroControlePNCP, empresaCNPJ, novoStatus) {
        try {
    ...
    extrairAnalisesAgentes(relatorioCompleto) {
        return {
    ...
    extrairAnaliseEstrategica(texto) {
    ...
    extrairAnaliseOperacional(texto) {
    ...
    extrairDadosOperacionais(texto) {
        const dados = {};
    ...
    extrairDocumentosNecessarios(texto) {
        const documentos = [];
    ...
    extrairDecisaoFinal(texto) {
        const decisaoMatch = texto.match(/DECISÃO FINAL:\s*([^\n]+)/i);
        return decisaoMatch ? decisaoMatch[1].trim() : 'NÃO DEFINIDA';
    }
    extrairNivelRisco(texto) {
