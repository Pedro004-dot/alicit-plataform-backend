...
export class LicitacaoPineconeService {
     ...
     */
    async indexLicitacao(licitacao) {
        try {
     ...
     */
    async buscarLicitacoesSimilares(consulta, topK = 20, filtros) {
        try {
            ...
            const resultados = await this.pineconeRepo.searchSimilar(queryEmbedding, undefined, // sem filtro de licitacaoId específica
            topK);
     ...
     */
    async indexarLicitacoesEmLote(licitacoes) {
     ...
     */
    criarDescricaoCompleta(licitacao) {
        const partes = [
        ...
        if (licitacao.itens && licitacao.itens.length > 0) {
            const itensPrincipais = licitacao.itens
                ...
                .map((item) => `${item.descricao} (${item.materialOuServicoNome})`)
                .join('; ');
    ...
    async buscarLicitacoesPorEstado(uf) {
        try {
            ...
            const licitacoesDoEstado = allLicitacoes.matches?.filter(match => String(match.metadata?.uf) === uf) || [];
