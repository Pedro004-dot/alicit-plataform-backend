...
export class PDFProcessorAdapter {
   ...
   */
  private async extractWithPdf2Json(document: PDFDocument): Promise<ProcessedPDFDocument> {
   ...
   */
  private async extractWithPdfParse(document: PDFDocument): Promise<ProcessedPDFDocument> {
    try {
   ...
   */
  async extractTextFromPDF(document: PDFDocument): Promise<ProcessedPDFDocument> {
  ...
  async extractTextFromMultiplePDFs(empresaId: string): Promise<ProcessedPDFDocument[]> {
    const results: ProcessedPDFDocument[] = [];
  ...
  generateEditalHash(empresaId: string): string {
    const combinedBuffer = Buffer.from(empresaId);
    return this.generateHash(combinedBuffer);
  }

  private extractTextFromData(pdfData: any): string {
    return pdfData.Pages.map((page: any) => {
      return page.Texts.map((text: any) => {
        const textContent = text.R.map((run: any) => decodeURIComponent(run.T)).join('');
  ...
  private convertToMarkdown(text: string, pdfData: any): string {
  ...
  private isSubtitle(line: string): boolean {
