...
export class PDFGeneratorAdapter {
  ...
  private async generateHTML(data: PDFReportData): Promise<string> {
...
<body>
  ...
  private extractPdfData(data: PDFReportData): any {
    const currentDate = new Date().toISOString();
    const formattedDate = new Date().toLocaleString('pt-BR');
    
    // Parser do markdown para dados frontend-friendly
    const parser = new MarkdownParser(data.finalReport);
    const dadosFrontend: DadosRelatorioFrontend = parser.parseToFrontendFormat(
      data.documentsAnalyzed,
      85, // qualityScore padrão
  ...
  private extractStructuredContent(content: string): any {
    const sections = [];
  ...
  private extractCheckmarks(content: string): { positivos: string[]; negativos: string[]; avisos: string[] } {
    const lines = content.split('\n');
  ...
  private extractExecutiveData(finalReport: string): any {
  ...
  private extractDocuments(content: string): any[] {
