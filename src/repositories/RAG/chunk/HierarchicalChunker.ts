...
export class HierarchicalChunker {
   ...
   */
  processDocument(text: string): HierarchicalChunkResult[] {
   ...
   */
  private extractProtoChunks(text: string): ProtoChunk[] {
    const protoChunks: ProtoChunk[] = [];
   ...
   */
  private detectHierarchy(line: string): { path: string; level: string } | null {
   ...
   */
  private consolidateProtoChunks(protoChunks: ProtoChunk[]): HierarchicalChunkResult[] {
    const finalChunks: HierarchicalChunkResult[] = [];
    let bufferText = '';
    let bufferMetadata = {
    ...
    for (const protoChunk of protoChunks) {
      ...
      if (bufferText.length + currentText.length + 2 > this.MAX_CHUNK_SIZE) {
        ...
        if (protoChunk.page) bufferMetadata.pages.add(protoChunk.page);
      } else {
   ...
   */
  private splitLargeChunk(text: string, hierarchyPath: string): HierarchicalChunkResult[] {
    const subChunks: HierarchicalChunkResult[] = [];
    const parts: string[] = [];
   ...
   */
  private createFinalChunk(bufferText: string, bufferMetadata: any): HierarchicalChunkResult {
    return {
      content: bufferText.trim(),
      metadata: {
        hierarchyPath: bufferMetadata.hierarchyPaths.join(' -> '),
        pages: Array.from(bufferMetadata.pages).sort() as number[],
   ...
   */
  private resetBuffer(bufferMetadata: any): void {
