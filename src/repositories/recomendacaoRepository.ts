...
class RecomendacaoRepository {
  async salvarRecomendacoes(recomendacoes: RecomendacaoData[]) {
    ...
    const licitacoesExistentesSet = new Set(
      licitacoesExistentes?.map(l => l.numero_controle_pncp) || []
    );
    
    const licitacoesFaltantes = numerosPNCP.filter(numeroPNCP => 
      !licitacoesExistentesSet.has(numeroPNCP)
    );
  ...
  async limparRecomendacoesAntigas(diasParaExpirar: number) {
  ...
  async verificarRecomendacaoExistente(numeroControlePNCP: string, empresaCnpj: string) {
    // Usar CNPJ com formatação original
