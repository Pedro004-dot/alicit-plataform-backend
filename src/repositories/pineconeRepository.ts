...
import { EditalChunk } from './editalRAGRepository';

export class PineconeRepository {
  private pinecone: Pinecone;
  private openaiClient: OpenAI;
  private indexName: string = 'alicit-editais';
  private vectorStore: Map<string, number[]> = new Map(); // Cache em memória para compatibilidade
  ...
  private async waitForIndex(indexName: string): Promise<void> {
   ...
   */
  async searchSimilar(queryEmbedding: number[], licitacaoId?: string, topK: number = 5): Promise<any[]> {
    try {
      const index = this.pinecone.index(this.indexName);
