...
export class EditalRAGRepository {
  private redisClient: any;
  private readonly TTL_SEMANA = 7 * 24 * 60 * 60; // 7 dias em segundos
  private openaiClient: OpenAI;
  private vectorStore: Map<string, number[]> = new Map(); // In-memory vector store
  ...
  async saveChunks(chunks: EditalChunk[], licitacaoId: string): Promise<void> {
    try {
      await this.connect();
  ...
  async hybridSearch(
    ...
    licitacaoId: string, 
    limit: number = 5,
    hybridWeight: number = 0.7 // Peso para busca vetorial vs keyword
  ): Promise<EditalChunk[]> {
    try {
      await this.connect();
  ...
  private enhancedKeywordScore(query: string, text: string): number {
  ...
  private expandQueryTerms(query: string): {term: string, weight: number}[] {
   ...
   */
  private calculateEnrichedStructuralBoost(chunkData: any, query: string): number {
    let boost = 0;
    ...
    boost += typeBoosts[sectionType as keyof typeof typeBoosts] || 0.05;
   ...
   */
  private queryMatchesContext(query: string, hierarchyPath: string, sectionType: string): boolean {
    const queryLower = query.toLowerCase();
    const pathLower = hierarchyPath.toLowerCase();
  ...
  async loadEmbeddingsFromRedis(licitacaoId: string): Promise<void> {
    try {
      await this.connect();
  ...
  private async generateEmbeddingForSearch(text: string): Promise<number[]> {
    try {
