...
interface CriarLicitacaoEmpresaData {
  cnpjEmpresa: string;
  numeroControlePNCP: string;
  status: string;
}

const criar = async (data: CriarLicitacaoEmpresaData) => {
...
const listarPorEmpresa = async (cnpj: string) => {
  ...
  const licitacoesMapeadas = (data || []).map((item: any) => ({
    id: item.id,
...
const buscarPorChaves = async (numeroControlePNCP: string, empresaCnpj: string) => {
  const { data, error } = await supabase
    .from('licitacoes_empresa')
    .select('*')
