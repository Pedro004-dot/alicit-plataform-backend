 ...
 */
async function getEmpresaDocuments(empresaId: string, categoria: string) {
  try {
 ...
 */
function compareDocumentRequirements(available: any[], required: any[]) {
  const comparison = {
    compliant: [] as any[],
    missing: [] as any[],
    expired: [] as any[],
  ...
  required.forEach(reqDoc => {
    const matchingDoc = available.find(availDoc => 
      availDoc.tipo.includes(reqDoc.tipo) || 
