...
import licitacaoEmpresaEstagiosRepository from "../../repositories/licitacaoEmpresaEstagiosRepository";

class DashboardService {
  async getDashboardData(cnpj: string) {
    ...
    licitacoes.forEach(licitacao => {
      ...
      if (status === 'em_analise') contadores.analise++;
      else if (status === 'aprovado') contadores.aguardando_confirmacao++;
      else if (status === 'nao_definido') contadores.analise++; // não_definido conta como análise
  ...
  async getLicitacoesComEstagios(cnpj: string) {
    ...
    return licitacoes.map((item: any) => ({
      id: item.id.toString(),
  ...
  private mapearStatusParaEstagio(status: string): string {
