import { PNCPLicitacao } from '../metrics/types';
import { FiltroGeografico } from './types';
...
const cidadesRaioCache = new Map<string, Set<string>>();
 ...
 */
export const filterLicitacoesPorGeografia = async (
  licitacoes: PNCPLicitacao[], 
  filtroGeo: FiltroGeografico
  ...
  if (cidadesRaioCache.has(cacheKey)) {
    ...
    return licitacoes.filter(lic => 
      cidadesPermitidas.has(lic.unidadeOrgao.municipioNome.toLowerCase())
    );
  }
  
  // Calcula e cacheia as cidades no raio
  const cidadesNoRaio = new Set<string>();
  const licitacoesFiltradas: PNCPLicitacao[] = [];
 ...
 */
export const clearCidadesRaioCache = (): void => {
