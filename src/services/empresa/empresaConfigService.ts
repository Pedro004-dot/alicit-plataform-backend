...
const atualizarConfiguracoesEmpresa = async (empresaId: string, dadosEmpresa: Partial<EmpresaInput>) => {
    try {
...
const buscarEmpresaPorCnpjCompleta = async (cnpj: string) => {
    try {
...
const processarDocumentosEmpresa = async (empresaId: string, documentos: any[]) => {
    try {
        const documentosProcessados = [];
        
        for (const doc of documentos) {
            if (doc.arquivo) {
                const documentoSalvo = await empresaRepository.uploadDocumento(
                    empresaId,
                    doc.arquivo,
                    doc.nomeDocumento,
                    doc.dataExpiracao
...
const putEmpresa = async (id: string, empresaInput: Partial<EmpresaInput>) => { 
