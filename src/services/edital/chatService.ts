...
export class EditalChatService {
  ...
  async chat(licitacaoId: string, query: string, empresaId: string): Promise<ChatResponse> {
    try {
  ...
  private async generateAnswer(query: string, context: string, licitacaoId: string): Promise<string> {
    try {
...
${context}`;
  ...
  private isContextIrrelevant(chunks: string[], topic: string): boolean {
  ...
  private async alternativeObjectSearch(licitacaoId: string): Promise<string[]> {
