...
import { PineconeStorage } from '../storage/PineconeStorage';

export class HybridSearch {
  private openaiClient: OpenAI;
  private vectorStorage: PineconeStorage;
  
  constructor(vectorStorage: PineconeStorage) {
  ...
  async search(
    ...
    licitacaoId: string,
    topK: number = 10,
    hybridWeight: number = 0.7 // Peso vetorial vs keyword
  ): Promise<string[]> {
    try {
   ...
   */
  private calculateStructuralBoost(chunkData: any, query: string): number {
    let boost = 0;
   ...
   */
  private queryMatchesHierarchy(query: string, hierarchyPath: string): boolean {
    const queryLower = query.toLowerCase();
