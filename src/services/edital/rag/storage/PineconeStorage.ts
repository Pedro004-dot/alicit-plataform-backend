...
export class PineconeStorage {
  ...
  private readonly TTL_SEMANA = 7 * 24 * 60 * 60; // Mantém para compatibilidade (não usado no Pinecone)
  private openaiClient: OpenAI;
  private vectorStore: Map<string, number[]> = new Map(); // Cache in-memory mantido
   ...
   */
  async loadEmbeddings(licitacaoId: string): Promise<void> {
    try {
   ...
   */
  async generateEmbeddings(chunks: EditalChunk[]): Promise<EditalChunk[]> {
    console.log(`🤖 Gerando embeddings para ${chunks.length} chunks...`);
    
    const chunksWithEmbeddings: EditalChunk[] = [];
    const BATCH_SIZE = 5;
   ...
   */
  async getChunkIds(licitacaoId: string): Promise<string[]> {
    try {
   ...
   */
  async getChunkData(chunkId: string): Promise<any | null> {
    try {
   ...
   */
  async searchSimilar(queryText: string, licitacaoId?: string, topK: number = 5): Promise<any[]> {
    try {
   ...
   */
  async searchSimilarWithEmbedding(queryEmbedding: number[], licitacaoId?: string, topK: number = 5): Promise<any[]> {
    try {
      return await this.pineconeRepo.searchSimilar(queryEmbedding, licitacaoId, topK);
    } catch (error) {
      console.error('❌ Erro na busca por similaridade:', error);
