import { EditalDocument, EditalChunk } from "../../../../repositories/editalRAGRepository";
import { HierarchicalChunker } from "../../../../repositories/RAG/chunk/HierarchicalChunker";

export async function chunkDocuments(documents: EditalDocument[]): Promise<EditalChunk[]> {
  const allChunks: EditalChunk[] = [];
  ...
  for (const document of documents) {
    ...
    try {
      ...
      if (hierarchicalChunks.length > 0) {
        ...
        const editalChunks: EditalChunk[] = hierarchicalChunks.map((hierChunk, index) => ({
 ...
 */
function calculateDepthFromPath(hierarchyPath: string): number {
 ...
 */
function calculateCriticality(content: string): number {
...
function createTraditionalChunks(document: EditalDocument): EditalChunk[] {
