...
var chunkSLDGELU7_cjs = require('./chunk-SLDGELU7.cjs');
...
async function validateAndSaveScore(storage, payload) {
  const payloadToSave = chunk2BFCQRWV_cjs.saveScorePayloadSchema.parse(payload);
...
_Mastra_decorators = [chunkBHB4PDNV_cjs.InstrumentClass({
  prefix: "mastra",
...
exports.Mastra = class Mastra {
  ...
  generateId() {
    if (this.#idGenerator) {
      ...
      if (!id) {
        const error = new chunkC73WLCY3_cjs.MastraError({
  ...
  constructor(config) {
    ...
    const workflowEventProcessor = new chunkSLDGELU7_cjs.WorkflowEventProcessor({
      mastra: this
    });
    ...
    if (config?.logger === false) {
      ...
      if (config?.logger) {
        ...
        const levelOnEnv = process.env.NODE_ENV === "production" && process.env.MASTRA_DEV !== "true" ? chunkV5WKCX3G_cjs.LogLevel.WARN : chunkV5WKCX3G_cjs.LogLevel.INFO;
        logger = new chunkV5WKCX3G_cjs.ConsoleLogger({
          name: "Mastra",
          level: levelOnEnv
        });
      }
    ...
    this.#telemetry = chunkBHB4PDNV_cjs.Telemetry.init(config?.telemetry);
...
`
  ...
  setTelemetry(telemetry) {
    this.#telemetry = chunkBHB4PDNV_cjs.Telemetry.init(telemetry);
    if (this.#agents) {
      Object.keys(this.#agents).forEach(key => {
        if (this.#telemetry) {
...
//# sourceMappingURL=chunk-YEZD4ZLX.cjs.map
//# sourceMappingURL=chunk-YEZD4ZLX.cjs.map