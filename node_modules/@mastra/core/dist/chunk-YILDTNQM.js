...
var AISDKV5OutputStream = class {
  ...
  async consumeStream(options) {
    try {
      await consumeStream({
        stream: this.fullStream.pipeThrough(new TransformStream({
...
var MastraModelOutput = class extends MastraBase {
  ...
  constructor({
    ...
    this.messageList = messageList;
    const self = this;
    this.#baseStream = stream.pipeThrough(new TransformStream({
...
var LegacyWorkflow = class extends MastraBase {
  ...
  loop(applyOperator, condition, fallbackStep, loopType, variables) {
    ...
    const checkStep = {
      ...
      outputSchema: z$1.object({
        status: z$1.enum(["continue", "complete"])
      })
    };
    const checkStepNode = {
      step: checkStep,
  ...
  async moderateContent(content, isStream = false, tracingContext) {
    ...
    try {
      ...
      const schema = z.object({
        category_scores: z.object(this.categories.reduce((props, category) => {
...
`) ?? void 0;
  ...
  async #execute({
    ...
    const prepareToolsStep = createStep({
      ...
      inputSchema: z$1.any(),
      outputSchema: z$1.object({
        convertedTools: z$1.record(z$1.string(), z$1.any())
      }),
      execute: async () => {
        const toolEnhancements = [
        // toolsets
