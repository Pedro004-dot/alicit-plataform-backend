...
var AISDKV5OutputStream = class {
  ...
  async consumeStream(options) {
    try {
      await aiV5.consumeStream({
        stream: this.fullStream.pipeThrough(new web.TransformStream({
  ...
  get fullStream() {
    ...
    const transformedStream = fullStream.pipeThrough(new web.TransformStream({
      transform(chunk, controller) {
...
var BaseFormatHandler = class {
  ...
  constructor(schema, options = {}) {
    if (!schema) {
      ...
      this.schema = aiV5.asSchema(schema);
    }
...
var ObjectFormatHandler = class extends BaseFormatHandler {
  ...
  async processPartialChunk({
    ...
    const {
      ...
      state
    } = await aiV5.parsePartialJson(accumulatedText);
...
var MastraModelOutput = class extends chunkKXCUCBEI_cjs.MastraBase {
  ...
  constructor({
    ...
    this.messageList = messageList;
    const self = this;
    this.#baseStream = stream.pipeThrough(new web.TransformStream({
  ...
  get fullStream() {
    ...
    const processorStates = /* @__PURE__ */new Map();
    return fullStream.pipeThrough(new web.TransformStream({
      async transform(chunk, controller) {
        if (self.processorRunner) {
          const {
            part: processedPart,
    ...
    })).pipeThrough(new web.TransformStream({
      transform(chunk, controller) {
   ...
   */
  get objectStream() {
    return this.fullStream.pipeThrough(new web.TransformStream({
      transform(chunk, controller) {
  ...
  get textStream() {
    ...
    return this.teeStream().pipeThrough(new web.TransformStream({
      transform(chunk, controller) {
...
var MastraLLMVNext = class extends chunkKXCUCBEI_cjs.MastraBase {
  #model;
  #mastra;
  constructor({
    model,
  ...
  _applySchemaCompat(schema) {
    ...
    if (model) {
      ...
      schemaCompatLayers.push(new schemaCompat.OpenAIReasoningSchemaCompatLayer(modelInfo), new schemaCompat.OpenAISchemaCompatLayer(modelInfo), new schemaCompat.GoogleSchemaCompatLayer(modelInfo), new schemaCompat.AnthropicSchemaCompatLayer(modelInfo), new schemaCompat.DeepSeekSchemaCompatLayer(modelInfo), new schemaCompat.MetaSchemaCompatLayer(modelInfo));
    }
...
function runScorer({
  ...
  chunkTSNDVBUU_cjs.executeHook("onScorerRun" /* ON_SCORER_RUN */, payload);
}
...
var WorkflowInstance = class {
  ...
  async runMachine(parentStepId, input, runtimeContext = new chunkGPWMM745_cjs.RuntimeContext()) {
  ...
  async resumeWithEvent(eventName, data, runtimeContext = new chunkGPWMM745_cjs.RuntimeContext()) {
  ...
  #makeStepDef(stepId) {
    ...
    const handler = async ({
      ...
      const {
        payload = {},
        execute: execute2 = async () => {}
      } = targetStep.step;
      const mergedData = {
        ...payload,
        ...context
      };
      const finalAction = this.#mastra?.getTelemetry() ? executeStep(execute2, `workflow.${this.name}.action.${stepId}`, {
        componentName: this.name,
        runId: rest.runId
      }) : execute2;
...
var LegacyWorkflow = class extends chunkKXCUCBEI_cjs.MastraBase {
  name;
  triggerSchema;
  resultSchema;
  resultMapping;
  ...
  loop(applyOperator, condition, fallbackStep, loopType, variables) {
    ...
    const checkStep = {
      ...
      outputSchema: z.z.object({
        status: z.z.enum(["continue", "complete"])
      })
    };
    const checkStepNode = {
      step: checkStep,
  ...
  #makeStepDef(stepId) {
    ...
    const handler = async ({
      ...
      const finalAction = this.telemetry ? executeStep(execute2, `workflow.${this.name}.action.${stepId}`, {
        componentName: this.name,
        runId: rest.runId
      }) : execute2;
      return finalAction ? await finalAction({
...
var ModerationProcessor = class _ModerationProcessor {
  ...
  constructor(options) {
    ...
    this.moderationAgent = new exports.Agent({
  ...
  async moderateContent(content, isStream = false, tracingContext) {
    ...
    try {
      ...
      const schema = z__default.default.object({
        category_scores: z__default.default.object(this.categories.reduce((props, category) => {
          props[category] = z__default.default.number().min(0).max(1).optional();
          return props;
        }, {})).optional(),
        reason: z__default.default.string().optional()
      });
      let response;
      if (model.specificationVersion === "v2") {
...
var PromptInjectionDetector = class _PromptInjectionDetector {
  ...
  constructor(options) {
    ...
    this.strategy = options.strategy || "block";
    this.includeScores = options.includeScores ?? false;
    this.detectionAgent = new exports.Agent({
      name: "prompt-injection-detector",
      instructions: options.instructions || this.createDefaultInstructions(),
      model: options.model
    });
    ...
    try {
      ...
      const schema = z__default.default.object({
        categories: z__default.default.object(this.detectionTypes.reduce((props, type) => {
          props[type] = z__default.default.number().min(0).max(1).optional();
          return props;
        }, {})).optional(),
        reason: z__default.default.string().optional(),
        rewritten_content: z__default.default.string().optional()
      });
      if (model.specificationVersion === "v2") {
...
var PIIDetector = class _PIIDetector {
  ...
  async detectPII(content, tracingContext) {
    ...
    const schema = z__default.default.object({
      ...
      detections: z__default.default.array(z__default.default.object({
        type: z__default.default.string(),
        value: z__default.default.string(),
        confidence: z__default.default.number().min(0).max(1),
        start: z__default.default.number(),
        end: z__default.default.number(),
        redacted_value: z__default.default.string().optional()
      })).optional(),
      redacted_content: z__default.default.string().optional()
    });
    try {
      const model = await this.detectionAgent.getModel();
      let response;
...
var LanguageDetector = class _LanguageDetector {
  ...
  async detectLanguage(content) {
    ...
    try {
      ...
      let response;
      const schema = z__default.default.object({
        iso_code: z__default.default.string().optional(),
        confidence: z__default.default.number().min(0).max(1).optional(),
        translated_text: z__default.default.string().optional()
      });
      if (model.specificationVersion === "v2") {
        response = await this.detectionAgent.generateVNext(prompt, {
          output: schema,
...
_Agent_decorators = [chunkBHB4PDNV_cjs.InstrumentClass({
  prefix: "agent",
...
exports.Agent = class Agent extends (_a = chunkKXCUCBEI_cjs.MastraBase) {
  id;
  name;
  #instructions;
  #description;
  ...
  constructor(config) {
    super({
      component: chunkV5WKCX3G_cjs.RegisteredLogger.AGENT
    });
    ...
    if (config.voice) {
      ...
      this.#voice = new chunkAHJY7RHO_cjs.DefaultVoice();
    }
  ...
  async getVoice({
    ...
    if (this.#voice) {
      ...
      return new chunkAHJY7RHO_cjs.DefaultVoice();
    }
  }
  get instructions() {
...
`) ?? void 0;
  ...
  async #runScorers({
    ...
    const runIdToUse = runId || this.#mastra?.generateId() || crypto2.randomUUID();
    if (Object.keys(this.evals || {}).length > 0) {
      for (const metric of Object.values(this.evals || {})) {
        chunkTSNDVBUU_cjs.executeHook("onGeneration" /* ON_GENERATION */, {
          input,
          output: outputText,
          runId: runIdToUse,
          metric,
  ...
  async prepareLLMOptions(messages, options, methodType) {
    ...
    const runId = args.runId || this.#mastra?.generateId() || crypto2.randomUUID();
    ...
    const activeSpan = chunkBHB4PDNV_cjs.Telemetry.getActiveSpan();
    ...
    if (Object.keys(baggageEntries).length > 0) {
      chunkBHB4PDNV_cjs.Telemetry.setBaggage(baggageEntries);
    }
    const memory = await this.getMemory({
      runtimeContext
    });
  ...
  async #execute({
    ...
    const prepareToolsStep = createStep({
      ...
      inputSchema: z.z.any(),
      outputSchema: z.z.object({
        convertedTools: z.z.record(z.z.string(), z.z.any())
      }),
      execute: async () => {
        const toolEnhancements = [
        // toolsets
    ...
    const streamStep = createStep({
      ...
      execute: async ({
        ...
        const outputProcessors = inputData.outputProcessors || (this.#outputProcessors ? typeof this.#outputProcessors === "function" ? await this.#outputProcessors({
          runtimeContext: inputData.runtimeContext || new chunkGPWMM745_cjs.RuntimeContext()
        }) : this.#outputProcessors : []);
  ...
  async speak(input, options) {
    ...
    try {
      ...
      if (e instanceof chunkC73WLCY3_cjs.MastraError) {
        ...
        err = new chunkC73WLCY3_cjs.MastraError({
...
exports.Agent = /*@__PURE__*/(_ => {
  ...
  exports.Agent = chunkEBVYYC2Q_cjs.__decorateElement(_init, 0, "Agent", _Agent_decorators, exports.Agent);
  chunkEBVYYC2Q_cjs.__runInitializers(_init, 1, exports.Agent);

  // src/stream/MastraWorkflowStream.ts
  return exports.Agent;
})();
var MastraWorkflowStream = class extends web.ReadableStream {
  #usageCount = {
    promptTokens: 0,
    completionTokens: 0,
    totalTokens: 0
...
var ExecutionEngine = class extends chunkKXCUCBEI_cjs.MastraBase {
  mastra;
  constructor({
    mastra
  }) {
    super({
      ...
      component: chunkV5WKCX3G_cjs.RegisteredLogger.WORKFLOW
    });
    this.mastra = mastra;
  }
  __registerMastra(mastra) {
...
var Workflow = class extends chunkKXCUCBEI_cjs.MastraBase {
  id;
  description;
  inputSchema;
  outputSchema;
...
var Run = class {
  ...
  async resume(params) {
    ...
    let runtimeContextToUse = params.runtimeContext ?? new chunkGPWMM745_cjs.RuntimeContext();
